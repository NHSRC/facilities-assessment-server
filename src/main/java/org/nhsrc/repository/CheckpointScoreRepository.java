...
public interface CheckpointScoreRepository extends TxDataRepository<CheckpointScore> {
    ...
    @Query("SELECT cl.name from CheckpointScore cs inner join cs.checklist cl where cs.facilityAssessment.id = :facilityAssessmentId group by cl.id")
    List<String> getFilledChecklists(@Param("facilityAssessmentId") Integer facilityAssessmentId);
}
