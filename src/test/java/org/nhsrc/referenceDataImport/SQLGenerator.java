            ...
            areaOfConcern.getStandards().forEach(standard -> {
                ...
                standard.getMeasurableElements().forEach(me -> {
                    ...
                    me.getCheckpoints().forEach(checkpoint -> {
                        ...
                        stringBuffer.append(String.format("insert into checkpoint (uuid, name, means_of_verification, measurable_element_id, checklist_id, am_observation, am_staff_interview, am_patient_interview, am_record_review, sort_order, is_optional, score_levels) values ('%s', '%s', '%s', (select measurable_element.id from measurable_element, assessment_tool where measurable_element.reference = '%s' and measurable_element.assessment_tool_id = assessment_tool.id and assessment_tool.name = '%s'), (select id from checklist where name = '%s' and assessment_tool_id = (select id from assessment_tool where name = '%s')), %b, %b, %b, %b, %b, %d, %b, %d);\n", UUID.randomUUID().toString(), checkpoint.getName().replace("'", "''"), mov, me.getReference(), assessmentToolName, checkpoint.getChecklist().getName(), data.getAssessmentTool().getName(), checkpoint.getAssessmentMethodObservation(), checkpoint.getAssessmentMethodStaffInterview(), checkpoint.getAssessmentMethodPatientInterview(), checkpoint.getAssessmentMethodRecordReview(), sortOrder.getAndIncrement(), checkpoint.isOptional(), checkpoint.getScoreLevels()));
                    });
                });
            });
        });
