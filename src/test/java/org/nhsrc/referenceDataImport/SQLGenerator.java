...
public class SQLGenerator {
    public void generate(AssessmentChecklistData data, File file) throws IOException {
        StringBuffer stringBuffer = new StringBuffer();
        generateState(data, stringBuffer);
    ...
    private void generateChecklistAreaOfConcernMapping(AssessmentChecklistData data, StringBuffer stringBuffer) {
        data.getChecklists().forEach(checklist -> {
            checklist.getAreasOfConcern().forEach(areaOfConcern -> {
                stringBuffer.append(String.format("insert into checklist_area_of_concern (area_of_concern_id, checklist_id) values ((select id from area_of_concern where reference = '%s'), (select id from checklist where name = '%s'));\n", areaOfConcern.getReference(), checklist.getName()));
            });
        });
    }
    ...
    private void generateAreaOfConcern(AssessmentChecklistData data, StringBuffer stringBuffer) {
        data.getAreaOfConcerns().forEach(areaOfConcern -> {
            stringBuffer.append(String.format("insert into area_of_concern (name, reference) values ('%s', '%s');\n", areaOfConcern.getName(), areaOfConcern.getReference()));
            areaOfConcern.getStandards().forEach(standard -> {
                stringBuffer.append(String.format("insert into standard (name, reference, area_of_concern_id) values ('%s', '%s', (select id from area_of_concern where reference = '%s'));\n", standard.getName(), standard.getReference(), areaOfConcern.getReference()));
                standard.getMeasurableElements().forEach(me -> {
                    stringBuffer.append(String.format("insert into measurable_element (name, reference, standard_id) values ('%s', '%s', (select id from standard where reference = '%s'));\n", me.getName().replace("'", "''"), me.getReference(), standard.getReference()));
                    me.getCheckpoints().forEach(checkpoint -> {
                        String mov = checkpoint.getMeansOfVerification() == null ? null : checkpoint.getMeansOfVerification().replace("'", "''");
                        stringBuffer.append(String.format("insert into checkpoint (name, means_of_verification, measurable_element_id, checklist_id, state_id, is_default, am_observation, am_staff_interview, am_patient_interview, am_record_review) values ('%s', '%s', (select id from measurable_element where reference = '%s'), (select id from checklist where name = '%s'), (select id from state where name = '%s'), %b, %b, %b, %b, %b);\n", checkpoint.getName().replace("'", "''"), mov, me.getReference(), checkpoint.getChecklist().getName(), data.getState().getName(), checkpoint.getDefault(), checkpoint.getAssessmentMethodObservation(), checkpoint.getAssessmentMethodStaffInterview(), checkpoint.getAssessmentMethodPatientInterview(), checkpoint.getAssessmentMethodRecordReview()));
                    });
                });
            });
        });
    ...
    private void generateState(AssessmentChecklistData data, StringBuffer stringBuffer) {
